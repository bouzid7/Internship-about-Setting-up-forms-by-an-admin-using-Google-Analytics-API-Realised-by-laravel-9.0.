...
$(function() {
        ...
        $('.choice-option').change(function()
         {
            ...
            else if (choice == "checkbox") 
            {
                ...
                add_checkbox()
                for (var i = 0; i < 3; i++) {
                    checkbox_field($(this), _field, "enter option",k+2)
                }
            } 
            else if (choice == "radio") 
            {
                ...
                for (var i = 0; i < 3; i++) {
                    pp++;
                    radio_field($(this), _field, "enter option",K+1,pp)
                }
            } 
            else if (choice == "file") 
            {
    ...
    function add_checkbox() {
        $('.add_chk').click(function() {
            ...
            checkbox_field(_this, _field, "Enter Option",k+2)
        })
    }
    ...
    function add_radio() {
        $('.add_radio').click(function() {
            ...
            var _this = $(this)
            var _field = _this.closest('.question-item').attr('data-item')
            radio_field(_this, _field, "Enter Option",k+1,pp)
        })
    }
    ...
    function checkbox_field(_this, _field, _text = "option",i) {
        var chk = $("<div>")
        var rem = $("<div>")
        chk.attr({
        ...
        var item = create_checkbox_field(_field, _text,i)
        chk.append(item)
    ...
    function radio_field(_this, _field, _text = "option",j,p) {
        var chk = $("<div>")
        var rem = $("<div>")
        chk.attr({
            "class": "col-sm-11 d-flex align-items-center",
        ...
        rem.append("<button class='btn btn-sm btn-default' type='button'><span class='fa fa-times'></span></button>")
        rem.attr('onclick', "$(this).closest('.row').remove()")
        var item = create_radio_field(_field, _text,j,p)
        chk.append(item)
        el = $("<div class='row w-100'>")
        el.append(rem)
        el.append(chk)
    ...
    function create_checkbox_field(_field, _text, i) {
    ...
    function create_radio_field(_field, _text,j,p) {
        var el = $('<div>')
        el.attr({
            "class": "form-check q-fc"
        })
    ...
    function save_form() 
    {
        ...
        $.ajax({
            url: "/admin/saveForm",
            method: "GET",
            data: { form_data: new_el.html(), description: description, title: title, form_code: form_code,id:id },
            dataType: "json",
            error: err => {
                console.log(err)
                alert("an error occured111111")
                location.href = "/admin"
            },
